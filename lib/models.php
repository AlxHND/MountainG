...
if(isset($_GET['fix_main_image'])) {
  ...
  $no_image_models = $models->noMainImageModels();
    // $hosting = "http://cdn.ct.sexhoundlinks.com";
...
?>
  Модель ID:<strong><?=intval($_REQUEST['deletemodel'])?></strong>, <strong><?=$models->getName()?></strong> не удалена. Ошибка!
<?php
  }
} else {
...
?>  
        <div style="float: left; padding-left: 15px;"><a href="index.php?act=models&amp;qrstring=<?=time()?>">Показать все списком</a></div>
<?php
  } else {
?>
        <div style="float: left; padding-left: 15px;"><a href="index.php?act=models&amp;thumbs=true&amp;qrstring=<?=time()?>">Показать все тумбами</a></div>
<?php    
  }
?>      
        <div style="float: left; padding-left: 15px; margin-left: 15px; border-left: solid 2px #000;">Тумбами:&nbsp;&nbsp;&nbsp;&nbsp;<a href="index.php?act=models&amp;thumbs=true&amp;sex=female&amp;qrstring=<?=time()?>">Female</a></div>
        <div style="float: left; padding-left: 10px;"><a href="index.php?act=models&amp;thumbs=true&amp;sex=male&amp;qrstring=<?=time()?>">Male</a></div>
        <div style="float: left; padding-left: 10px;"><a href="index.php?act=models&amp;thumbs=true&amp;sex=shemale&amp;qrstring=<?=time()?>">Shemale</a></div>
        <div style="float: left; padding-left: 15px; margin-left: 15px; border-left: solid 2px #000;">Списком:&nbsp;&nbsp;&nbsp;&nbsp;<a href="index.php?act=models&amp;sex=female&amp;qrstring=<?=time()?>">Female</a></div>
        <div style="float: left; padding-left: 10px;"><a href="index.php?act=models&amp;sex=male&amp;qrstring=<?=time()?>">Male</a></div>
        <div style="float: left; padding-left: 10px;"><a href="index.php?act=models&amp;sex=shemale&amp;qrstring=<?=time()?>">Shemale</a></div>
        <div style="padding-right: 30px; float: right; display: block-inline;">&lt; <a href="index.php?act=models_import&amp;qrstring=<?=time()?>">Импорт моделей</a> | <a href="index.php?act=models&amp;query=add&amp;qrstring=<?=time()?>">Добавить модель</a> &gt;</div>
</div>
<?php
...
?>
      <script>
        ...
        function get_model_images(model_id, thumb_layout) {
            ...
            if (!check_if_opened) {
              $jq.post("util/model.show_all_images.php", {model_id : model_id, thumb_layout : thumb_layout}, function(data){
                if ('success' in data) {
                  var node = document.getElementById('wrapper');
                  var close_div = document.createElement('div');
                  close_div.setAttribute("style","width: 100%; position: absolute; top: 306px; left: 283px; color: white; background-color: rgb(255, 90, 90); width: 200px; height: 33px; text-align: center; font-size: 26px; cursor: pointer;");
                  ...
                  img_div.setAttribute("style", "position:fixed; width: auto; height: auto; background-color: rgba(0, 0, 0, 0.6); border: solid 1px #000;");
                  ...
                  for (var image in data.images) {
                    img_div.innerHTML += '<img src='+url_pre + data.images[image]+' onclick="set_model_main_image('+model_id+', '+image+', \''+thumb_layout+'\')" style="margin: 15px;" />';
                  }
        ...
        function set_model_main_image(model_id, image_id, thumb_layout) {
              ...
              $jq.post("util/model.set_main_image.php", {model_id : model_id, image_id : image_id, thumb_layout : thumb_layout}, function(data){
                if ('success' in data) {
      ...
      <form enctype="multipart/form-data" action="<?=$_SERVER['SCRIPT_NAME'] . "?". $_SERVER['QUERY_STRING']?>" method="post" id="modelform">
        <div align="center">
<?php
      if (isset($thumbURL)) {
?>
        ...
        <table class="disclaim" cellpadding="3" cellspacing="3" border="0">
          ...
          <tr>
            ...
            <td bgcolor="#e4e4e4">
                ...
                <input type="radio" name="active" value="yes"<?php if ($active == 'yes' || !$active) echo " checked"; ?>>Да
            </td>
          </tr>
          <tr>
            <td bgcolor="#e4e4e4">Пол: </td>
            <td bgcolor="#e4e4e4">
                <input type="radio" name="sex" value="female"<?php if ($sex == 'female' || !$sex) echo " checked"; ?>>Жен.
                <input type="radio" name="sex" value="male"<?php if ($sex == 'male') echo " checked"; ?>>Муж.
                <input type="radio" name="sex" value="shemale"<?php if ($sex == 'shemale') echo " checked"; ?>>Шмель
            </td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td bgcolor="#e4e4e4">
              <select name="ethnic">
                <option value="none"<?php if (!$ethnic || ($ethnic && $ethnic == 'none')) echo " selected='selected'"; ?>>Неизвестно</option>
                <option value="american"<?php if ($ethnic && $ethnic == 'american') echo " selected='selected'"; ?>>american</option>
                <option value="arab"<?php if ($ethnic && $ethnic == 'arab') echo " selected='selected'"; ?>>arab</option>
                <option value="asian"<?php if ($ethnic && $ethnic == 'asian') echo " selected='selected'"; ?>>asian</option>
                <option value="ebony"<?php if ($ethnic && $ethnic == 'ebony') echo " selected='selected'"; ?>>ebony</option>
                <option value="euro"<?php if ($ethnic && $ethnic == 'euro') echo " selected='selected'"; ?>>euro</option>
                <option value="indian"<?php if ($ethnic && $ethnic == 'indian') echo " selected='selected'"; ?>>indian</option>
                <option value="latin" <?php if ($ethnic && $ethnic == 'latin') echo " selected='selected'"; ?>>latin</option>
              </select>
            </td>
          </tr>
          <tr>
            ...
            <td bgcolor="#e4e4e4">
              <select name="eyes">
                <option value="none"<?php if (!$eyes || ($eyes && $eyes == 'none')) echo " selected='selected'"; ?>>Неизвестно</option>
                <option value="amber"<?php if ($eyes && $eyes == 'amber') echo " selected='selected'"; ?>>amber</option>
                <option value="blue" <?php if ($eyes && $eyes == 'blue') echo " selected='selected'"; ?>>blue</option>
                <option value="brown"<?php if ($eyes && $eyes == 'brown') echo " selected='selected'"; ?>>brown</option>
                <option value="gray"<?php if ($eyes && $eyes == 'gray') echo " selected='selected'"; ?>>gray</option>
                <option value="green"<?php if ($eyes && $eyes == 'green') echo " selected='selected'"; ?>>green</option>
                <option value="hazel"<?php if ($eyes && $eyes == 'hazel') echo " selected='selected'"; ?>>hazel</option>
              </select>
            </td>
          </tr>                    
          <tr>
            ...
            <td bgcolor="#e4e4e4">
              <input type="radio" name="role" value="bottom"<?php if ($role == 'bottom') echo " checked"; ?>>Bottom
              <input type="radio" name="role" value="top"<?php if ($role == 'top') echo " checked"; ?>>Top
              <input type="radio" name="role" value="versatile"<?php if ($role == 'versatile' || !$role) echo " checked"; ?>>Versatile
            </td>
          </tr>
          <tr>
            ...
            <td bgcolor="#e4e4e4">
              <select name="hair">
                <option value="bald"<?php if ($hair == 'bald') echo " selected='selected'"; ?>>bald</option>
                <option value="blond" <?php if ($hair == 'blond') echo " selected='selected'"; ?>>blond</option>
                <option value="brown"<?php if ($hair == 'brown' || !$hair) echo " selected='selected'"; ?>>brown</option>
                <option value="brunette"<?php if ($hair == 'brunette') echo " selected='selected'"; ?>>brunette</option>
                <option value="gray"<?php if ($hair == 'gray') echo " selected='selected'"; ?>>gray</option>
                <option value="red"<?php if ($hair == 'red') echo " selected='selected'"; ?>>red</option>
                <option value="white"<?php if ($hair == 'white') echo " selected='selected'"; ?>>white</option>
              </select>
            </td>
          </tr>
          <tr>
            ...
            <td bgcolor="#e4e4e4">
              <select name="body">
                <option value="none"<?php if ($body && $body == 'none') echo " selected='selected'"; ?>>Неизвестно</option>
                <option value="skinny" <?php if ($body && $body == 'skinny') echo " selected='selected'"; ?>>skinny</option>
                <option value="thin"<?php if ($body && $body == 'thin') echo " selected='selected'"; ?>>thin</option>
                <option value="slim"<?php if ($body && $body == 'slim') echo " selected='selected'"; ?>>slim</option>                
                <option value="athletic"<?php if ($body && $body == 'athletic') echo " selected='selected'"; ?>>athletic</option>
                <option value="muscular"<?php if ($body && $body == 'muscular') echo " selected='selected'"; ?>>muscular</option>
                <option value="bodybuilder"<?php if ($body && $body == 'bodybuilder') echo " selected='selected'"; ?>>bodybuilder</option>
                <option value="chubby"<?php if ($body && $body == 'chubby') echo " selected='selected'"; ?>>chubby</option>
                <option value="fat"<?php if ($body && $body == 'fat') echo " selected='selected'"; ?>>fat</option>
              </select>
            </td>
          </tr>
          <tr>
            ...
            <td bgcolor="#e4e4e4">
              <select name="zodiac">
                <option value="none"<?php if ($zodiac == 'none' || !$zodiac) echo " selected='selected'"; ?>>Неизвестно</option>
<?php 
                foreach ($zodiac_signs as $zodiac_elem) { ?>                  
                  <option value="<?=$zodiac_elem?>"<?php if ($zodiac == $zodiac_elem) echo " selected='selected'"; ?>><?=$zodiac_elem?></option>
<?php           }     ?>
              </select>
            </td>
          </tr>
          <tr>
            ...
            <td bgcolor="#e4e4e4">
              <select name="category_of_age">
                <option value="Неизвестно"<?php if ($category_of_age == -1) echo " selected='selected'"; ?>>Неизвестно</option>
                <option value="20" <?php if ($category_of_age == 20) echo " selected='selected'"; ?>>18..20</option>
                <option value="25"<?php if ($category_of_age == 25) echo " selected='selected'"; ?>>20..25</option>
                <option value="35"<?php if ($category_of_age == 35) echo " selected='selected'"; ?>>25..35</option>                
                <option value="45"<?php if ($category_of_age == 45) echo " selected='selected'"; ?>>35..45</option>
                <option value="55"<?php if ($category_of_age == 55) echo " selected='selected'"; ?>>45..55</option>
                <option value="65"<?php if ($category_of_age == 65) echo " selected='selected'"; ?>>55..65</option>
                <option value="90"<?php if ($category_of_age == 90) echo " selected='selected'"; ?>>65+</option>
              </select>
            </td>
          </tr> 
          <tr>
            ...
            <td bgcolor="#e4e4e4">
              <input type="radio" name="cock_boobs" value="none"<?php if ($cock_boobs == 'none' || !$cock_boobs) echo " checked"; ?>>Неизвестно
              <input type="radio" name="cock_boobs" value="natural"<?php if ($cock_boobs == 'natural') echo " checked"; ?>>Натуральные/Необрезан
              <input type="radio" name="cock_boobs" value="mod"<?php if ($cock_boobs == 'mod') echo " checked"; ?>>Силикон/Обрезан
            </td>
          </tr>
          <tr>
            <td bgcolor="#e4e4e4">Пирсинг: </td>
            <td bgcolor="#e4e4e4">
              <input type="radio" name="piercing" value="none"<?php if ($piercing == 'none' || !$piercing) echo " checked"; ?>>Неизвестно
              <input type="radio" name="piercing" value="yes"<?php if ($piercing == 'yes') echo " checked"; ?>>Есть
              <input type="radio" name="piercing" value="no"<?php if ($piercing == 'no') echo " checked"; ?>>Нет
            </td>
          </tr>
          <tr>
            <td bgcolor="#e4e4e4">Татту: </td>
            <td bgcolor="#e4e4e4">
              <input type="radio" name="tattoo" value="none"<?php if ($tattoo == 'none' || !$tattoo) echo " checked"; ?>>Неизвестно
              <input type="radio" name="tattoo" value="yes"<?php if ($tattoo == 'yes') echo " checked"; ?>>Есть
              <input type="radio" name="tattoo" value="no"<?php if ($tattoo == 'no') echo " checked"; ?>>Нет
            </td>
          </tr>          
          <tr>
            <td bgcolor="#e4e4e4">Описание тату: </td>
            <td bgcolor="#e4e4e4"><input size="42" name="tattooDesc" <?php if ($tattooDesc) echo "value='".$tattooDesc."'";?>></td>
          </tr>         
          ...
          <tr>
            <td bgcolor="#e4e4e4" colspan="2" style="text-align: center;">
              ...
              <div style="margin:10px; float: left;" id="select_vertic_tumb"  onclick="get_model_images(<?=$id?>, 'vertic')">
              ...
              <div style="margin:10px; float: left;" id="select_horiz_tumb"  onclick="get_model_images(<?=$id?>, 'horiz')">
...
?>              
                  <div id="pseudonim<?=$idX?>" style="width:100%; height: 34px; display: block-inline;">
                    <dd style="float: left; padding: 5px; margin: 2px;"><input size="30" name='inputPseudonim<?=$idX?>' id='inputPseudonim<?=$idX?>' onkeyup="preChangePseudonim(<?=$idX?>)" value='<?=$pseudonim?>'></dd>
                    <dd style="float: left; padding: 0; margin: 2px;"><img src="images/button_red_minus.png" border=0 onclick="removePseudonim(<?=$idX?>);" /></dd>
                  </div>
<?php
                  }
                }
?>
                <div id="addPseudonim0" style="width:100%; height: 34px; display: block-inline;">
                  <dd style="float: left; padding: 5px; margin: 2px; display: block;"><input size="30" name='addPseudonimName0' id='addPseudonimName0' value=''></dd>
                  <dd style="float: left; padding: 0; margin: 2px; display: block;"><img src="images/add_button_small.png" border=0 onclick="addPseudonim(<?=$id?>);" /></dd>
                </div>
<?php              
            } else {
?>
                <div id="addPseudonim0" style="width:100%; height: 34px; display: block-inline;">
                  <dd style="float: left; padding: 5px; margin: 2px; display: block;"><input size="30" name='addPseudonimName0' id='addPseudonimName0' value=''></dd>
                  <dd style="float: left; padding: 0; margin: 2px; display: block;"><img src="images/add_button_small.png" border=0 onclick="addPseudonim_newModel();" /></dd>
                </div>
<?php              
            }               
?>      
...
?>
          <div style="float: left; color: red;">Удалить модель <input type="checkbox" id="deletemodel" value="<?=intval($_GET['id'])?>" name="deletemodel"></div>
<?php          
        }
?>        
        <input type="submit" <?php if(isset($_GET['id'])) { ?> onclick="return checkModelDelete(<?=$_GET['id']?>)" <?php } ?> value="<?php if ($id) { ?>Изменить модель<?php } else { ?>Добавить модель<?php } ?>" name="<?php if ($id) { ?>updateModel<?php } else { ?>addModel<?php } ?>" />
        </div>
      </div>
...
<?php          
      ...
      if (isset($_GET['id'])) {
?>
        <form enctype="multipart/form-data" action="<?=$_SERVER['SCRIPT_NAME']."?act=models&id=".$_GET['id']."&search=".$_GET['id']?>" method="post">
              <br /><hr /><br />
              Найти все возможные галеры с участием модели, исключая уже добавленные: <input type="submit" value="Поиск">
              <br /><hr /><br />
        </form>
        <form enctype="multipart/form-data" action="<?=$_SERVER['SCRIPT_NAME']."?act=models&id=".$_GET['id']."&search=".$_GET['id']."&pseudo=true"?>" method="post">
              <br /><hr /><br />
              Найти все возможные галеры ТОЛЬКО С ПСЕВДОНИМАМИ модели, исключая уже добавленные: <input type="submit" value="Поиск">
              <br /><hr /><br />
        </form>  
          <br />
<?php
...
?>
         <div style="margin:6px; padding: 5px; width: 1300px; height: 26px; border: 1px #000 solid; display: block-inline; text-align: left;">
          <div style='float:left; width: 800px; height: 24px; display: block-inline; overflow: hidden; '>
            <input type="button" id="showGalleryButton<?=$galId?>" value="Показать все тумбы" onclick="showGalleryThumbs(<?=$galId?>,'<?=$gallery['type']?>');">
            <strong>ID:</strong> <a  onmouseover="over('<?=$thumbURL?>')" onmousemove="move(event)" onmouseout="out()" href="./index.php?act=galleries&galid=<?=$galId?>"><?=$galId?></a> | 
            <strong>Платник:</strong> <?=$gallery['paysite']?>
            <strong>Тайтл:</strong> <?=$gallery['title']?>
          </div>
          <div style='float:right;'>
            <input type="button" value="Добавить" id="addModelToGallery<?=$galId?>" onclick="addModelToGallery(<?=$galId?>,<?=$id?>);">
            <input type="button" value="Удалить" id="deleteModelFromGallery<?=$galId?>" onclick="deleteModelFromGallery(<?=$galId?>,<?=$id?>);" style="display: none;">
          </div>
         
        </div>
        <div style="margin:6px; margin-top: 1px; margin-bottom: 25px; padding: 5px; width: 1300px; height: auto; border: 1px #000 solid; display: none; text-align: left;" name="addModelBlock" id="addModelBlock<?=$galId?>">
...
<?php
  ...
  ?>
          <a href="index.php?act=models&amp;id=<?=$model ['id_model']?>">
            <div style="margin:6px; padding: 3px; width: 186px; height: <?=$site_id ? 295 : 276?>px; border: 1px #000 solid; display: block-inline; float:left; text-align: center;">
...
<?php                  
  ...
  ?>
            <div style="margin:6px; padding: 5px; width: 1200px; height: 18px; border: 1px #000 solid; display: block-inline;">
              <div style='float:left;'>
                <a onmouseover="over('<?=$thumbURL?>')" onmousemove="move(event)" onmouseout="out()" href="<?=$_SERVER['SCRIPT_NAME']?>?act=models&id=<?=$model ['id_model']?>">Edit model</a> | 
              ...
              <div style='float:right'>
                Платник: <?=$model['personal_site_id'] ? "<a href=\"index.php?act=paysites&amp;siteid=".$model['personal_site_id']."&amp;edit\">".$paysite['name']."</a>" : 'Нет'?>, Галер: <strong><?=$models->getModelGalsCount($model ['id_model'])?></strong>
              </div>
            </div>
