...
class Maker
{
	...
	private function SelectTags ($tags,$type, $sort = 'asc', $accept_gifs = false, $language = false, $gals_count = 50) {
		...
		if($this->max_times_used_gals !== false) {
			if($this->max_times_used_gals == 0) { $max_times_used_gals = " AND times_used_on_sites = 0 "; }
			elseif($this->max_times_used_gals) { $max_times_used_gals = " AND times_used_on_sites < ".$this->max_times_used_gals." "; }
		}
		...
		if ($tags['category'] == 0 && (!isset($tags['exclude_tag']) || !$tags['exclude_tag'])) {
			if ($this->dont_show_used) { $dont_show_used = " AND galleries.gal_id NOT IN ( SELECT gal_id FROM sites_galleries ) "; }
			else { $dont_show_used = ""; }
	...
	public function __construct ($site, $type, $sort = 'desc', $dont_show_used = false, $only_no_empty_titles = false, $gals_count = 50, $exclude_tag_from_make = 0, $with_tag = 0) {
		...
		if(isset($site['thumb_by_horiz_width']) && $site['thumb_by_horiz_width']) { $this->thumb_by_horiz_width = true;	}
