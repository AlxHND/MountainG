...
*/
	...
	class CacheRebuilder {
		...
		private function galleryGlobalKey($id) { return $this->default_prefix."gallery:" . intval($id); }
		private function modelGlobalKey($id) { return $this->default_prefix."model:" . intval($id); }
		private function siteGlobalKey($id) { return $this->default_prefix."site:" . intval($id); }
		private function sourceGlobalKey($id) { return $this->default_prefix."source:" . intval($id); }
		// база тегов есть и по имени, и по айди (дублированная, следует пересмотреть необходимость)
		private function tagGlobalKey($id) { return $this->default_prefix."tag:" . $id; }

		private function modelsDBKey() { return $this->default_prefix."models";	}
		private function galleriesDBKey() { return $this->default_prefix."galleries";	}
		private function tagsDBKey() { return $this->default_prefix."tags";	}

		// смена с tagsDBKey siteTagsDBKey
		private function siteTagsDBKey($site_id) { return $this->default_prefix.(int)$site_id.":unique_tags";	}
		// tagGlobalKey
		private function siteUniqueTagKey($site_id, $tag_id) { return $this->default_prefix.(int)$site_id.":unique_tag:" . $tag_id; }


		private function sourcesDBKey() { return $this->default_prefix."sources";	}

		private function galleryLocalInfoKey ($site_id, $gal_id) { // $gallery_local_key_prefix
		...
		private function localGalleriesDB($site_id) { // ключ списка local_id:global_id
			$key_prefix = $this->default_prefix . $site_id .":";
			$key = $key_prefix . "galleries"; 
			return $key;
		}

		private function no_localGalleriesDB($site_id) { // ключ списка global_id:local_id
			$key_prefix = $this->default_prefix . $site_id .":";
		...
		private function keyFor($list_of, $sort_type, $content_type, $site_id, $item_id = false) {
			...
			if (!preg_match("#(galleries|tags|sources|models|model_galleries|tag_galleries|source_galleries|likes_galleries)#", $list_of) ||
...
*/	
		...
		*/
		...
		function updateStatsInDb($site_id, $update_object, $update_what) {
			...
			if(preg_match("#^(likes|pageviews)$#", $update_what)
...
*/		
		...
		//зарядить рейтинг
		function initializeSiteGalleriesRating($site_id) { // не доделано
			$result = false;
			$server_changed = false;
			$previous_server = false;
			$site_switched = $this->sites->switchSite($site_id);
			if ($site_switched) {
				...
				if ($this->connected) {
					...
					foreach ($galleries as $local_id => $gallery) {
								if (isset($gallery['id']) && isset($gallery['global_id']) && isset($gallery['gal_type']) 
...
*/
	...
	public function addGalleryTransaction($site_id, $gal_global_id, $gal_local_id, $gal_type, $added_on, &$pipeline) { // проверить зачем функция
		$result = false;
		$site_id = intval($site_id);
		$key_prefix = $this->default_prefix.$site_id.":";
		$galleries_db_key = $key_prefix."galleries";
...
function fixOKgalleries () {
		...
		function server_cacheGlobalGallery($gal_id) {
			// $result = false;
			// $previous_server = $this->current_server;
			// $servers_count = intval(REDIS_SERVERS_COUNT);
			// for ($i=0; $i <= $servers_count; $i++) {
			// 	$gallery_cached = false;
			// 	if ($i != $previous_server) $this->redis_connection($i);
			// 	if ($this->connected) {
			// 		try {
			...
			// 		}
			// 		if ($gallery_cached) {
			// 			$log = new Logger(__METHOD__.": GID#".$gal_id." закешировано на сервере #".$i);
			// 			$result = true;
			// 		} else {
			// 			echo "Ошибка! Не закешировано на сервере #".$i."<br>";
			// 			$log = new Logger ("Не закешировано на сервере #".$i.", галера: ".$gal_id, true);
			// 		}
			// 	} else $log = new Logger ("Нет коннекта к редису #".$i, true);
			// }
			// $this->redis_connection(0);
			// return $result;
		...
		function server_cacheModel($model_id) {
			...
			// 		} else {
			// 			//echo "Ошибка! Не закешировано на сервере #".$i."<br>";
			// 			$log = new Logger ("Не закешировано на сервере #".$i.", модель: ".$model_id, true);
			// 		}
			// 	} else $log = new Logger ("Нет коннекта к редису #".$i, true);
			// }
			// $this->redis_connection(0);
		...
		function server_cacheTag($tag_id) {
			...
			// 	if ($i != $previous_server) $this->redis_connection($i);
			// 	if ($this->connected) {
			// 		if ($this->cacheTag($tag_id)) $result = true;
			// 		else {
			// 			echo "Ошибка! Не закешировано на сервере #".$i."<br>";
			// 			$log = new Logger ("Не закешировано на сервере #".$i.", тег: ".$tag_id, true);
			// 		}
			// 	} else $log = new Logger ("Нет коннекта к редису #".$i, true);
			// }
			// $this->redis_connection(0);
