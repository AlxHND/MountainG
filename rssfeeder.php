...
$pwrd = 'gfdE4wqkjHgdo';
$content_url = 'xrhost.com/uploadedimages';
...
if (isset($_GET['model_info'])) {
	...
	$modelsArray = $modelService->find_models_by_string($_GET['model_info']);
...
if (isset($_GET['sitemaps'])) {
	...
	if (isset($_GET['site_id'])) {
		...
		if ($site && is_array($site)) {
			...
			// $tags = $sites_util->getSiteTagsListings($site['site_name']);
			// $sources = $sites_util->getSiteSourcesListings($site['site_name']);
			// $models = $sites_util->getSiteModelsListings($site['site_name']);
		}
