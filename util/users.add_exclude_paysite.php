...
$auth->requireAdminJson('Ошибка аутентификации при добавлении исключения палтника из выдачи юзера. Нужны права администратора.');

if (isset($_POST['paysite_id']) && isset($_POST['user_id'])) {
	$user_id = $_POST['user_id'];
	$paysite_id = $_POST['paysite_id'];

	$paysite_added = $userAuth->addExcludePaysite($user_id, $paysite_id);
	if ($paysite_added) {
		$string = json_encode(
			array(
				'success' => $user_id,
